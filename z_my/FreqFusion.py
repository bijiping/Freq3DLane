...
class FreqFusion(nn.Module):
    def __init__(self,
                 ...
                 feature_resample=False ,  # use offset generator or not False
...
if __name__ == '__main__':
    ...
    # 创建输入张量并将其移动到 GPU
    x = torch.rand(1, 128, 18, 32).to(device)
    x2 = torch.rand(1, 128, 9, 16).to(device)
    # mask = torch.rand(4, 4 * 25, 16, 16)
    ff1 = FreqFusion(hr_channels=128, lr_channels=128).to(device)
    y = ff1(x,x2)
    print(y[0].shape)
    print(y[1].shape)
    print(y[2].shape)
    total_params = sum(p.numel() for p in ff1.parameters())
